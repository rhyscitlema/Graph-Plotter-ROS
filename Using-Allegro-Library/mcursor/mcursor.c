...
#include "../graphplotter/camera.h"
#include "../hardware/mouse.h"
#include "../hardware/screen.h"


// current and previous mouse cursor postion
// note: relavite to camera origin and local axes
int mcursorPos[2];
     ...
     for(i=0; i<MCI_XSize; i++)
      if(i+x<SCR.XRes && j+y<SCR.YRes)
       if(mcursor_image[j][i]!=0xFF000000)
        my_putpixel (i+x, j+y, mcursor_image[j][i]);
}
